...
import { 
  Store, Package, GitCompare, AlertTriangle, CheckCircle, XCircle,
  Search, Download, RefreshCw, X, Eye, TrendingUp, Check, Link2, Unlink,
  BarChart3, Settings, Layers, ArrowUpDown
} from 'lucide-react'
import './App.css'
...
function App() {
  ...
  const [stats, setStats] = useState({
    restaurantes: 0, itens_geraldo: 0, itens_ifood: 0, sincronizados: 0,
    pendentes: 0, divergencias: 0, categorias: 0, precos: 0, matched: 0
  })
  ...
  async function loadRestaurantes() {
    ...
    rests?.forEach(r => map.set(r.id, { ...r, cats_geraldo: 0, cats_ifood: 0, itens_geraldo: 0, itens_ifood: 0, health: 0, matched: 0 }))
    ...
    matchesData?.forEach(m => {
      ...
      if (r && (m.status === 'confirmado' || m.status === 'auto')) r.matched++
    })
    ...
    map.forEach(r => {
      ...
      if (r.matched > r.itens_geraldo * 0.5) h++
      r.health = h
    })
  ...
  async function loadStats() {
    ...
    const { count: matchCount } = await supabase.from('item_matches').select('*', { count: 'exact', head: true }).in('status', ['confirmado', 'auto'])

    const sinc = rests?.filter(r => r.ifood_uuid).length || 0
    setStats({
      restaurantes: rests?.length || 0, itens_geraldo: itemG || 0, itens_ifood: itemI || 0,
      sincronizados: sinc, pendentes: (rests?.length || 0) - sinc, divergencias: 0,
      categorias: (catG || 0) + (catI || 0), precos: precos || 0, matched: matchCount || 0
    })
  }

  async function loadItens(restId?: number) {
    let query = supabase.from('itens').select('*, categorias(nome, origem, restaurante_id, restaurantes(nome)), precos(valor)').limit(1000)
    if (restId) {
    ...
    setItens(data?.map((i: any) => ({
      ...i, categoria_nome: i.categorias?.nome, restaurante_nome: i.categorias?.restaurantes?.nome,
      origem: i.categorias?.origem, preco: i.precos?.[0]?.valor || 0
    })) || [])
  }
  ...
  async function loadDivergencias() {
    ...
    gItens?.forEach((g: any) => {
      ...
      if (!match) {
        divs.push({ restaurante: g.categorias.restaurantes?.nome || '', geraldo_id: g.categorias.restaurantes?.geraldo_id || 0,
          item_geraldo: g.nome, item_ifood: '-', preco_geraldo: pG, preco_ifood: 0, tipo: 'ausente_ifood' })
      } else if (Math.abs(pG - pI) > 0.5) {
        divs.push({ restaurante: g.categorias.restaurantes?.nome || '', geraldo_id: g.categorias.restaurantes?.geraldo_id || 0,
          item_geraldo: g.nome, item_ifood: match.nome, preco_geraldo: pG, preco_ifood: pI, tipo: 'preco' })
      }
    })
  ...
  async function openPanel(rest: Restaurante) {
    ...
    if (catIds.length) {
      ...
      setPanelItens(data?.map((i: any) => ({
        ...i, categoria_nome: i.categorias?.nome, origem: i.categorias?.origem, preco: i.precos?.[0]?.valor || 0
      })) || [])
    }
  }

  async function loadMatching(rest: Restaurante) {
    ...
    let itensGeraldo: any[] = []
    if (catGIds.length) {
      const { data } = await supabase.from('itens').select('*, precos(valor)').in('categoria_id', catGIds)
      itensGeraldo = data?.map((i: any) => ({ id: i.id, nome: i.nome, descricao: i.descricao || '', categoria_id: i.categoria_id,
        categoria_nome: catGMap.get(i.categoria_id) || '', preco: i.precos?.[0]?.valor || 0 })) || []
    }
    ...
    if (catIIds.length) {
      ...
      itensIfood = data?.map((i: any) => ({ id: i.id, nome: i.nome, descricao: i.descricao || '', categoria_id: i.categoria_id,
        categoria_nome: catIMap.get(i.categoria_id) || '', preco: i.precos?.[0]?.valor || 0 })) || []
    }
    ...
    const matchList: Match[] = itensGeraldo.map(g => {
      ...
      if (existing) {
        return { id: existing.id, item_geraldo_id: g.id, item_ifood_id: existing.item_ifood_id, confianca: existing.confianca,
          status: existing.status, match_por: existing.match_por, geraldo: g, ifood: itensIfood.find(i => i.id === existing.item_ifood_id) || null }
      }

      let bestMatch: any = null, bestScore = 0, matchPor = ''
      for (const ifood of itensIfood) {
        ...
        let score = 0, por = ''
        if (catSim >= 80 && nomeSim === 100) { score = 100; por = 'nome+cat' }
        else if (catSim >= 80 && nomeSim >= 90) { score = 95; por = 'nome+cat' }
        else if (catSim >= 80 && nomeSim >= 80) { score = Math.abs(g.preco - ifood.preco) < 1 ? 90 : 85; por = 'nome+cat' }
        else if (nomeSim >= 95) { score = 80; por = 'nome' }
        else if (catSim >= 80 && nomeSim >= 60) { score = 70; por = 'nome+cat' }
        if (score > bestScore) { bestScore = score; bestMatch = ifood; matchPor = por }
      }
      return { item_geraldo_id: g.id, item_ifood_id: bestMatch?.id || null, confianca: bestScore,
        status: bestScore >= 90 ? 'auto' : 'pendente', match_por: matchPor, geraldo: g, ifood: bestMatch }
    })
  ...
  async function saveMatch(match: Match, newStatus: string) {
    ...
    if (match.id) {
      await supabase.from('item_matches').update({ status: newStatus, updated_at: new Date().toISOString() }).eq('id', match.id)
    } else {
      await supabase.from('item_matches').insert({ restaurante_id: selectedRest!.id, item_geraldo_id: match.item_geraldo_id,
        item_ifood_id: match.item_ifood_id, confianca: match.confianca, status: newStatus, match_por: match.match_por })
    }
    setMatches(prev => prev.map(m => m.item_geraldo_id === match.item_geraldo_id ? { ...m, status: newStatus } : m))
    setSavingMatch(null)
  }

  async function saveAllAuto() {
    for (const m of matches.filter(m => m.confianca >= 90 && m.status !== 'confirmado')) await saveMatch(m, 'confirmado')
  ...
  async function unlinkMatch(match: Match) {
    setSavingMatch(match.item_geraldo_id)
    if (match.id) {
      await supabase.from('item_matches').update({ item_ifood_id: null, confianca: 0, status: 'sem_match', updated_at: new Date().toISOString() }).eq('id', match.id)
    } else {
      await supabase.from('item_matches').insert({ restaurante_id: selectedRest!.id, item_geraldo_id: match.item_geraldo_id,
        item_ifood_id: null, confianca: 0, status: 'sem_match', match_por: 'manual' })
    }
    setMatches(prev => prev.map(m => m.item_geraldo_id === match.item_geraldo_id ? { ...m, item_ifood_id: null, ifood: null, confianca: 0, status: 'sem_match' } : m))
    setSavingMatch(null)
  }
  ...
  function exportCSV() {
    ...
    if (view === 'restaurantes') {
      csv = 'Nome,ID,Itens Geraldo,Itens iFood,Matched,Status,Saude\n' +
        filtered.map(r => `"${r.nome}",${r.geraldo_id},${r.itens_geraldo},${r.itens_ifood},${r.matched},${r.ifood_uuid ? 'OK' : 'Pendente'},${r.health}/5`).join('\n')
  ...
  const matchStats = { total: matches.length, confirmados: matches.filter(m => m.status === 'confirmado' || (m.status === 'auto' && m.confianca >= 90)).length,
      ...
      <main className="main">
        ...
        <div className="content">
          {loading ? (
            ...
            <>
              <div className="stats-row">
                ...
                <div className="stat-card">
                  <div className="stat-header"><div className="stat-icon blue"><Package size={20} /></div><span className="stat-trend up">Geraldo</span></div>
                  <div className="stat-value">{stats.itens_geraldo.toLocaleString()}</div>
                  <div className="stat-label">Itens Geraldo</div>
                </div>
                <div className="stat-card">
                  <div className="stat-header"><div className="stat-icon red"><Package size={20} /></div><span className="stat-trend down">iFood</span></div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-sub">{stats.restaurantes > 0 ? ((stats.sincronizados / stats.restaurantes) * 100).toFixed(0) : 0}% do total</div>
                </div>
                <div className="stat-card">
              ...
              <div className="filters-bar">
                ...
                <button className="btn btn-secondary" onClick={() => { setView('divergencias'); loadDivergencias() }}><AlertTriangle size={16} /> Ver Divergências</button>
                <button className="btn btn-secondary" onClick={() => { setView('itens'); loadItens() }}><Package size={16} /> Ver Todos Itens</button>
                <button className="btn btn-primary" onClick={() => setView('restaurantes')}><Store size={16} /> Ver Restaurantes</button>
              </div>

              <div className="table-container">
                <div className="table-header"><h3>Restaurantes com Problemas</h3></div>
                <table>
                  <thead><tr><th>Restaurante</th><th>Itens G</th><th>Itens iF</th><th>Matched</th><th>Saúde</th><th>Status</th></tr></thead>
                  <tbody>
                      ...
                      <tr key={r.id} onClick={() => openPanel(r)} style={{ cursor: 'pointer' }}>
                        ...
                        <td>
                          <div className="health-bar">
                            {[...Array(5)].map((_, i) => (<div key={i} className={`health-segment ${i < r.health ? (r.health >= 4 ? 'filled' : r.health >= 2 ? 'warning' : 'danger') : ''}`} />))}
                          </div>
                        </td>
            ...
            <>
              <div className="filters-bar">
                <div className="search-box"><Search size={18} /><input placeholder="Buscar restaurante..." value={search} onChange={e => setSearch(e.target.value)} /></div>
                <div className="filter-chips">
                  {(['todos', 'ok', 'pendente', 'problema'] as FilterStatus[]).map(f => (
                    <div key={f} className={`chip ${filterStatus === f ? 'active' : ''}`} onClick={() => setFilterStatus(f)}>
                      {f === 'todos' && <Layers size={14} />}{f === 'ok' && <CheckCircle size={14} />}{f === 'pendente' && <AlertTriangle size={14} />}{f === 'problema' && <XCircle size={14} />}
                      {f.charAt(0).toUpperCase() + f.slice(1)}
                      <span className="chip-count">{f === 'todos' ? stats.restaurantes : f === 'ok' ? stats.sincronizados : f === 'pendente' ? stats.pendentes : restaurantes.filter(r => r.health < 3).length}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th>Matched</th><th>Saúde</th><th>Status</th><th></th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map(r => (
                      <tr key={r.id}>
                        ...
                        <td>{r.ifood_uuid ? <span className="badge badge-success"><CheckCircle size={12} /> Sincronizado</span> : <span className="badge badge-warning"><AlertTriangle size={12} /> Pendente</span>}</td>
                        <td><button className="btn btn-ghost btn-sm" onClick={() => openPanel(r)}><Eye size={16} /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <>
              <div className="filters-bar">
                <div className="search-box"><Search size={18} /><input placeholder="Buscar item..." value={search} onChange={e => setSearch(e.target.value)} /></div>
                <div className="filter-group"><label>Origem:</label>
                  <select className="filter-select" value={filterOrigem} onChange={e => setFilterOrigem(e.target.value)}>
                    <option value="todos">Todas</option><option value="geraldo">Geraldo</option><option value="ifood">iFood</option>
                  </select>
                </div>
              </div>
              <div className="table-container">
                ...
                <table>
                  <thead><tr><th>Item</th><th>Restaurante</th><th>Categoria</th><th>Origem</th><th>Preço</th></tr></thead>
                  <tbody>
            ...
            <>
              <div className="stats-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                <div className="stat-card"><div className="stat-header"><div className="stat-icon red"><AlertTriangle size={20} /></div></div><div className="stat-value">{divergencias.length}</div><div className="stat-label">Total Divergências</div></div>
                <div className="stat-card"><div className="stat-header"><div className="stat-icon yellow"><TrendingUp size={20} /></div></div><div className="stat-value">{divergencias.filter(d => d.tipo === 'preco').length}</div><div className="stat-label">Preços Diferentes</div></div>
                <div className="stat-card"><div className="stat-header"><div className="stat-icon blue"><XCircle size={20} /></div></div><div className="stat-value">{divergencias.filter(d => d.tipo === 'ausente_ifood').length}</div><div className="stat-label">Ausente no iFood</div></div>
              </div>
              <div className="filters-bar"><div className="search-box"><Search size={18} /><input placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} /></div></div>
              <div className="table-container">
                <div className="table-header"><h3>Divergências Encontradas</h3></div>
                <table>
                  <thead><tr><th>Restaurante</th><th>Item Geraldo</th><th>Item iFood</th><th>R$ Geraldo</th><th>R$ iFood</th><th>Tipo</th></tr></thead>
                  <tbody>
        ...
        {selectedRest && (
          <>
            ...
            <div className="panel-tabs">
              <div className={`panel-tab ${panelTab === 'info' ? 'active' : ''}`} onClick={() => setPanelTab('info')}>📋 Info</div>
              <div className={`panel-tab ${panelTab === 'matching' ? 'active' : ''}`} onClick={() => { setPanelTab('matching'); if (matches.length === 0) loadMatching(selectedRest) }}>
                🔗 Matching <span className="panel-tab-badge">{selectedRest.matched}/{selectedRest.itens_geraldo}</span>
              </div>
            </div>
              ...
              {panelTab === 'info' ? (
                <>
                  <div className="side-panel-section">
                    ...
                    <div className="info-grid">
                      <div className="info-item"><div className="info-item-label">Geraldo ID</div><div className="info-item-value">{selectedRest.geraldo_id}</div></div>
                      <div className="info-item"><div className="info-item-label">Saúde</div><div className="info-item-value">{selectedRest.health}/5</div></div>
                      <div className="info-item"><div className="info-item-label">Categorias</div><div className="info-item-value">{selectedRest.cats_geraldo} G / {selectedRest.cats_ifood} iF</div></div>
                      <div className="info-item"><div className="info-item-label">Itens</div><div className="info-item-value">{selectedRest.itens_geraldo} G / {selectedRest.itens_ifood} iF</div></div>
                    </div>
                    ...
                    <div className="items-list">
                      {panelItens.map(i => (
                        <div key={i.id} className="item-row">
                          <div className="item-row-origem"><span className={`badge badge-sm ${i.origem === 'geraldo' ? 'badge-info' : 'badge-danger'}`}>{i.origem === 'geraldo' ? 'G' : 'iF'}</span></div>
                ...
                <>
                  ...
                  <div className="match-stats">
                    <div className="match-stat"><span className="match-stat-value">{matchStats.total}</span><span>Total</span></div>
                    <div className="match-stat green"><span className="match-stat-value">{matchStats.confirmados}</span><span>✓ OK</span></div>
                    <div className="match-stat yellow"><span className="match-stat-value">{matchStats.pendentes}</span><span>⚠ Pend</span></div>
                    <div className="match-stat red"><span className="match-stat-value">{matchStats.sem}</span><span>✗ Sem</span></div>
                  </div>

                  <div className="match-filters">
                    {(['todos', '100', 'pendente', 'sem'] as const).map(f => (
                      <div key={f} className={`match-filter ${filterMatch === f ? 'active' : ''}`} onClick={() => setFilterMatch(f)}>
                        {f === 'todos' ? 'Todos' : f === '100' ? '100%' : f === 'pendente' ? 'Pend' : 'Sem'}
                      </div>
                    ))}
                  </div>

                  <button className="btn btn-primary btn-block" onClick={saveAllAuto} style={{ marginBottom: 12 }}>
                    <Check size={16} /> Confirmar Auto ({matches.filter(m => m.confianca >= 90 && m.status !== 'confirmado').length})
                  </button>

                  {loadingMatches ? <div className="loading"><div className="spinner"></div></div> : (
                    <div className="match-list-panel">
                      {filteredMatches.map(m => (
                        <div key={m.item_geraldo_id} className={`match-item ${m.status === 'confirmado' ? 'confirmed' : m.confianca >= 90 ? 'auto' : m.confianca > 0 ? 'pending' : 'nomatch'}`}>
                          <div className="match-item-left">
                            <div className="match-item-name">{m.geraldo.nome}</div>
                            <div className="match-item-cat">{m.geraldo.categoria_nome} • R$ {m.geraldo.preco.toFixed(2)}</div>
                          </div>
                          <div className="match-item-center">
                            <div className={`match-item-score ${m.confianca >= 90 ? 'high' : m.confianca > 0 ? 'mid' : 'low'}`}>
                          ...
                          <div className="match-item-right">
                            {m.ifood ? (
                              <>
                                ...
                                <div className="match-item-cat">{m.ifood.categoria_nome} • R$ {m.ifood.preco.toFixed(2)}</div>
                              </>
                            ) : <div className="match-item-empty">Sem match</div>}
                          </div>
                          <div className="match-item-actions">
                            {m.status !== 'confirmado' && m.confianca > 0 && (
                              <button className="btn btn-sm btn-success" onClick={() => saveMatch(m, 'confirmado')} disabled={savingMatch === m.item_geraldo_id}><Check size={12} /></button>
                            )}
                            {m.ifood && (
                              <button className="btn btn-sm btn-ghost" onClick={() => unlinkMatch(m)} disabled={savingMatch === m.item_geraldo_id}><Unlink size={12} /></button>
                            )}
                          </div>
                        </div>
                      ))}
